...
case 'enable':
	...
	header('Location: webseer_mail.php?header=false');
	exit;
...
function form_actions() {
	...
	if (cacti_sizeof($url_array)) {
		...
		raise_message(40);
		header('Location: webseer_mail.php');
		exit;
	}
...
function form_save() {
	...
	header('Location: webseer_mail.php?action=edit&id=' . $id . '&header=false');
	exit;
}
...
function webseer_edit_url() {
	...
	form_save_button('webseer_mail.php', 'return');
...
function webseer_show_history() {
	...
	$refresh['page']    = 'webseer_mail.php?action=history&id=' . get_filter_request_var('id') . '&header=false';
	...
	$nav = html_nav_bar('webseer_mail.php?action=history', MAX_DISPLAY_PAGES, get_request_var('page'), $rows, $total_rows, cacti_sizeof($display_text), __('Urls', 'webseer'), 'page', 'main');

	webseer_show_tab('webseer_mail.php');
	...
	html_header_sort($display_text, get_request_var('sort_column'), get_request_var('sort_direction'), 1, 'webseer_mail.php?action=history&id=' . get_request_var('id'), 'main');
...
function list_urls() {
	...
	$nav = html_nav_bar('webseer_mail.php', MAX_DISPLAY_PAGES, get_request_var('page'), $rows, $total_rows, $columns, __('Checks', 'webseer'), 'page', 'main');

	form_start('webseer_mail.php', 'chk');
	...
	if (cacti_sizeof($result)) {
		foreach ($result as $row) {
			...
			print "<td width='1%' style='padding:0px;white-space:nowrap'>
				<a class='pic' href='" . html_escape($config['url_path'] . 'plugins/webseer/webseer_mail.php?action=edit&id=' . $row['id']) . "' title='" . __esc('Edit Service Check', 'webseer') . "'>
					<i class='tholdGlyphEdit fas fa-wrench'></i>
				</a>";

			if ($row['enabled'] == '') {
				print "<a class='pic' href='" . html_escape($config['url_path'] . 'plugins/webseer/webseer_mail.php?action=enable&id=' . $row['id']) . "' title='" . __esc('Enable Service Check', 'webseer') . "'>
					<i class='tholdGlyphEnable fas fa-play-circle'></i>
				</a>";
			} else {
				print "<a class='pic' href='" . html_escape($config['url_path'] . 'plugins/webseer/webseer_mail.php?action=disable&id=' . $row['id']) . "' title='" . __esc('Disable Service Check', 'webseer') . "'>
					<i class='tholdGlyphDisable fas fa-stop-circle'></i>
				</a>";
			}

			print "<a class='pic' href='" . html_escape($config['url_path'] . 'plugins/webseer/webseer_mail.php?action=history&id=' . $row['id']) . "' title='" . __esc('View Service Check History', 'webseer') . "'>
...
function webseer_filter() {
	...
	$refresh['page']    = 'webseer_mail.php?header=false';
	...
	function applyFilter() {
		strURL  = 'webseer_mail.php?header=false&state=' + $('#state').val();
	...
	function clearFilter() {
		strURL = 'webseer_mail.php?clear=1&header=false';
		loadPageNoHeader(strURL);
	}
	...
	html_start_box(__('Webseer Service Checks', 'webseer') , '100%', '', '3', 'center', 'webseer_mail.php?action=edit');
	?>
	<tr class='even noprint'>
		<td class='noprint'>
			<form id='form_webseer' action='webseer_mail.php'>
...
function webseer_log_filter() {
	...
	function applyFilter() {
		strURL  = 'webseer_mail.php?action=history&header=false&id=<?php print get_request_var('id');?>';
	...
	function clearFilter() {
		strURL = 'webseer_mail.php?action=history&clear=1&header=false';
		loadPageNoHeader(strURL);
	}

	function purgeEvents() {
		strURL = 'webseer_mail.php?action=purge&id=<?php print get_request_var('id');?>';
		loadPageNoHeader(strURL);
	}
	...
	<tr class='even noprint'>
		<td class='noprint'>
			<form id='webseer' action='webseer_mail.php?action=history'>
			<table class='filterTable'>
				<tr class='noprint'>
					<td>
